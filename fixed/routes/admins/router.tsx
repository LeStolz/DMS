...
adminsRouter.post("/search", admin, async (req, res) => {
  ...
  try {
    users = (
      await (await req.db()).input("phone", phone).execute("getUsersFixed")
    ).recordset;
  } catch {}
...
adminsRouter.post("/toggleLock", admin, async (req, res) => {
  ...
  try {
    await (
      ...
      .input("phone", phone)
      .input("role", role)
      .execute(`${lock === "on" ? "lockUserFixed" : "unlockUserFixed"}`);
  } catch {}
    ...
    const user: User = {
      ...(
        await (
          ...
          .input("gender", gender)
          .execute(
            `${role === "dentist" ? "createDentistFixed" : "createStaffFixed"}`
          )
      ).recordset[0],
      role,
    };

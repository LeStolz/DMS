...
dentistsRouter.post("/search", dentist, async (req, res) => {
  ...
  try {
    users = (
      await (await req.db())
        .input("phone", phone)
        .execute("getPatientsByPhoneFixed")
    ).recordset;
  } catch (error: any) {
    return res.send(
  ...
  try {
    const patientDetails = await parseSqlJson(
      (
        await (await req.db())
          .input("id", req.params.userId)
          .execute("getPatientDetailsFixed")
      ).recordset[0]
    );
...
dentistsRouter.get("/addTreatment", dentist, async (req, res) => {
  ...
  try {
    services = (await (await req.db()).execute("getServicesFixed")).recordset;
      ...
      services.map(
        async (service) =>
          (
            await (await req.db())
              .input("id", service.id)
              .execute("getServiceDetailsFixed")
          ).recordset[0]
      )
    );
  } catch {}
  ...
  try {
    drugs = (await (await req.db()).input("name", "").execute("getDrugsFixed"))
      .recordset;
...
dentistsRouter.post("/addTreatment", dentist, async (req, res) => {
  ...
  try {
      ...
      for (let serviceId of serviceIds) {
        try {
          await (await req.db())
            ...
            .execute("addServiceToTreatmentFixed");
        } catch {}
      }
    })();
      ...
      drugIds.map(async (drug) => {
        try {
          await (await req.db())
            ...
            .execute("addDrugToTreatmentFixed");
        } catch (error: any) {
          if (error instanceof Error) {
            if (
    ...
    await (await req.db())
      .input("id", treatmentId)
      .execute("saveTreatmentFixed");
...
dentistsRouter.get("/schedule/:date", dentist, async (req, res) => {
  ...
  try {
    schedules = (
      await (await req.db())
        ...
        .execute("getDentistSchedulesFixed")
    ).recordset;
  } catch {}
  ...
  try {
    appointments = (
      await (await req.db())
        .input("id", req.user?.id)
        .execute("getDentistAppointmentsFixed")
    ).recordset;
  } catch {}
  ...
  try {
    if (req.body.date == null) {
      await (await req.db())
        ...
        .execute("addDentistScheduleFixed");
    } else {
      await (await req.db())
        .input("id", req.user?.id)
        .input("date", req.body.day)
        .input("shift", req.body.shift)
        .execute("removeDentistScheduleFixed");
    }
  } catch (error: any) {
    if (error instanceof Error) {
      return res.status(400).send(formatError(error.message));

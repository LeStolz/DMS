...
usersRouter.get("/", async (req, res) => {
  ...
  try {
    dentists = (await (await req.db()).execute("getDentistsFixed")).recordset;
      ...
      dentists.map(
        async (dentist) =>
          (
            await (await req.db())
              .input("id", dentist.id)
              .execute("getDentistDetailsFixed")
          ).recordset[0]
      )
    );
  } catch {}
    ...
    try {
      user = (
        await (await req.db())
          ...
          .execute("createGuestPatientFixed")
      ).recordset[0];
    } catch (error: any) {
  ...
  try {
    dentists = (
      await (await req.db())
        .input("date", date)
        .input("shift", shift)
        .execute("getDentistsOnShiftFixed")
    ).recordset;
  } catch {}

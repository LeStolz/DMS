...
create or alter proc getUserByCredFixed(
	@phone nchar(10),
...
create or alter proc getPatientsByPhoneFixed(@phone nchar(10)) as
begin tran
	set xact_abort on
	set nocount on
	...
	begin try
		...
		if not exists(
			select * from patient with(tablockx)
...
create or alter proc createPatientFixed(
	@name nvarchar(64),
	@password nvarchar(64),
	@phone nchar(10),
	@gender nvarchar(8),
...
create or alter proc createGuestPatientFixed(
	@name nvarchar(64),
	@phone nchar(10),
	@gender nvarchar(8),
	@dob date,
...
create or alter proc createStaffFixed(
	@name nvarchar(64),
	@password nvarchar(64),
	@phone nchar(10),
	@gender nvarchar(8)
...
create or alter proc createDentistFixed(
	@name nvarchar(64),
	@password nvarchar(64),
	@phone nchar(10),
	@gender nvarchar(8)
...
create or alter proc lockUserFixed(
	@phone nchar(10),
	@role nvarchar(16)
)
with execute as owner
...
create or alter proc unlockUserFixed(
	@phone nchar(10),
	@role nvarchar(16)
)
with execute as owner
...
begin tran
	...
	begin try
		if not exists(select * from dentist with (tablock, xlock))
		begin;
...
create or alter proc getDentistDetailsFixed(@id uniqueidentifier) as
begin tran
	set xact_abort on
	set nocount on
...
create or alter proc getDentistSchedulesFixed(@id uniqueidentifier) as
begin tran
	set xact_abort on
	set nocount on
	set transaction isolation level serializable
	...
	begin try
		...
		if not exists (select * from dentistSchedule with(tablock, xlock) where dentistId = @id)
		begin;
...
create or alter proc getDentistAppointmentsFixed(@id uniqueidentifier) as
begin tran
	set xact_abort on
	set nocount on
	set transaction isolation level serializable
	...
	begin try
		...
		if not exists (select * from appointment with(tablock, xlock) where dentistId = @id)
		begin;
...
create or alter proc getUsersFixed(@phone nchar(10)) as
begin tran
	set xact_abort on
	set nocount on
...
create or alter proc bookAppointmentFixed(
...
create or alter proc getDentistsOnShiftFixed(
	@date date,
	@shift nvarchar(16)
) as
begin tran
...
create or alter proc updatePatientFixed(
	@id uniqueidentifier,
	@name nvarchar(64),
	@phone nchar(10),
	@gender nvarchar(8),
...
create or alter proc getPatientDetailsFixed(@id uniqueidentifier) as
begin tran
	set xact_abort on
	set nocount on
	set transaction isolation level repeatable read
...
create or alter proc addDentistScheduleFixed(
	@id uniqueidentifier,
	@date int,
	@shift nvarchar(16)
) as
...
create or alter proc removeDentistScheduleFixed(
	@id uniqueidentifier,
	@date int,
	@shift nvarchar(16)
) as
...
create or alter proc getDrugsFixed(@name nvarchar(64)) as
begin tran
	set xact_abort on
	set nocount on
	set transaction isolation level serializable
	...
	begin try
		...
		if not exists(
			select * from drug with(tablock, xlock)
...
create or alter proc getDrugDetailsFixed(@id uniqueidentifier) as
begin tran
	set xact_abort on
	set nocount on
	set transaction isolation level serializable
...
create or alter proc getServiceDetailsFixed(@id uniqueidentifier) as
begin tran
	set xact_abort on
	set nocount on
...
create or alter proc createDrugFixed(
	@name nvarchar(64),
...
create or alter proc updateDrugFixed(
	@id uniqueidentifier,
	@name nvarchar(64),
	@directive nvarchar(512),
	@price int,
...
begin tran
	...
	begin try
		...
		declare @oldPrice int = (select price from drug with(tablock, xlock) where id = @id)
...
create or alter proc deleteDrugFixed(@id uniqueidentifier) as
begin tran
	set xact_abort on
	set nocount on
	set transaction isolation level serializable
...
create or alter proc addDrugBatchFixed(
...
create or alter proc removeDrugBatchFixed(
	@drugId uniqueidentifier,
	@exp date
) as
begin tran
	...
	begin try
		...
		if not exists(
			select * from drugBatch with(tablock, xlock) where drugId = @drugId and expirationDate = @exp and isRemoved = 0
		)
		begin;
...
create or alter proc createInvoiceFixed(@treatmentId uniqueidentifier) as
begin tran
	set xact_abort on
	set nocount on
...
create or alter proc createTreatmentFixed(
	@dentistId uniqueidentifier,
	@shift nvarchar(16),
	@date date,
...
create or alter proc addServiceToTreatmentFixed(
...
begin tran
	...
	begin try
		...
		if not exists(select * from treatment with(tablock, xlock) where id = @treatmentId)
		begin;
			throw 51000, 'Treatment does not exist.', 1
		end
...
create or alter proc addDrugToTreatmentFixed(
	@treatmentId uniqueidentifier,
	@drugId uniqueidentifier,
	@expirationDate date,
	@dosage nvarchar(64),
...
create or alter proc saveTreatmentFixed(@id uniqueidentifier) as
begin tran
	set xact_abort on
	set nocount on
